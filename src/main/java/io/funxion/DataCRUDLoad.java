...
public class DataCRUDLoad extends TestCase {
	private static String BASE_URL,APIKEY;

	public static void main(String[] args) {
		DataCRUDLoad testCase = new DataCRUDLoad();
	...
	protected void execute() throws Exception {
		...
		JsonObject book = createBook(APIKEY);
		endStep("CREATEBOOK");
		debug("Book ISBN : ".concat(book.getString("identifier")));
		Thread.sleep(100);
		
		startStep("READ");
		String price = readBook(APIKEY,book.getString("identifier"));
		...
		startStep("UPDATE");
		Integer dbId = updateBook(APIKEY,book.getString("identifier"));
		endStep("UPDATE");
		//Assert(dbId,"2","UnExpected ID");
		Thread.sleep(100);

		startStep("READUPDATED");
		String newprice = readBook(APIKEY,book.getString("identifier"));
		...
		startStep("DELETE");
		Integer count = deleteBook(APIKEY,book.getString("identifier"));
		Assert(count,1,"UnExpected Count");
		endStep("DELETE");
		Thread.sleep(100);
		
		/*startStep("DROP");
		Integer dropCount = deleteBookType(APIKEY);
